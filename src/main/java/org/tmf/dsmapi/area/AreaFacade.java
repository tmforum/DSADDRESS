...
//    StateModelImpl stateModel = new StateModelImpl();
    ...
    public void checkCreation(Area entity) throws BadUsageException, UnknownResourceException {

        Area sc = null;
        if (entity.getId() == null
                || entity.getId().isEmpty()) {
//            throw new BadUsageException(ExceptionType.BAD_USAGE_GENERIC, "While creating Area, id must be not null");
                //Do nothing create ok
                Logger.getLogger(AreaFacade.class.getName()).log(Level.INFO, "Area with autogenerated id is being posted");
        } else {
            try {
                sc = this.find(entity.getId());
                if (null != sc) {
                    throw new BadUsageException(ExceptionType.BAD_USAGE_GENERIC,
                            "Duplicate Exception, Area with same id :" + entity.getId() + " alreay exists");
                }
            } catch (UnknownResourceException ex) {
                //Do nothing create ok
                Logger.getLogger(AreaFacade.class.getName()).log(Level.INFO, "Area with id = " + entity.getId() + " is being posted", ex);
            }
        }
    ...
    public Area patchAttributs(String id, Area partialEntity) throws UnknownResourceException, BadUsageException {
        Area currentEntity = this.find(id);
    ...
    public void verifyStatus(Area currentEntity, Area partialEntity) throws BadUsageException {
        /**
